...
- 2026-09-07 (round 2): external reviewer confirmed Table 1 (208 session values within 3e-14; raw-voxel recomputation within 1.5e-5) and all 52 masks; accepted ROI-average calibration as primary. Six S2 + four S3 findings accepted and fixed: manifests recorded a pre-revision HEAD (now HEAD + dirty flag + Scripts tree hash, and a re-run under committed code), hand-typed S4 numbers (supplement now generated by 11_manuscript_numbers.py), missing muscle-offset limitation (added), completeness/Dice wording, stale z-profile summaries (marked informational with mask hashes), doc overstatements, run_pipeline first-match CT, cache checks for tissue/body tasks, muscle success with zero valid voxels, calibration fallback labelling. Scientific choices unchanged.
